...
Decision_make_distance = 7.5 # during the test, lane is visible between 4m-6.5m
...
class LaneTracker:
    ...
    def update(self, real_world_coords, pc, pc_l, pc2veh, pc2veh_l, pc2world, pc2world_l, gps, heading, ph, ph_l, rv, lv):
        ...
        ## J 0318 (avoid heading read error)
        # if heading>0:
        if heading:
            self.history['veh_heading'].append(heading)
        else:
            try:
                default = self.history['veh_heading'][-1]
                self.history['veh_heading'].append(default)
            except:
                print("heading read error")
                self.history['veh_heading'].append(0)    
...
def edge_det_mask_range(frame, hough_thr, blur_thr, mask):
    ...
    # edges = cv2.Canny(masked_input, 50, 150) # prev 50, 150 for cloudy; 120,200 for sunny
    edges = cv2.Canny(masked_input, 150, 200)
...
while True:
    ...
    ## init vehicle status
    v2w_heading = None # v2p for camera stream ui, v2w for execution
    current_gps = None
    gps0 = None
