## dan --> provide no 360/0, try -180,180 (no big deal)
...
def path_upload_x5_trailer():
	...
	Rv = 0.82	# Half-width of tags
	L_sv = 1.23  # trailer sensor to hinge
	L_ht = 0.37  # Hinge to 2 Tag Line center distance 
	yh = -2.9   # Hinge to vehicle center
	...
	print("--J est. 0804 uwb decawave (trailer2veh)--", d1, d2, theta_rad)
	'''
	...
	for i in range(200):
		...
		p_trailer_h = global_to_local_rotate(theta_rad, [curve_points[i][0], curve_points[i][1]]) # p or n
		points_trailer_dan_trailer_h.append([p_trailer_h[0], p_trailer_h[1], curve_points[i][2]+theta_rad, 'r']) # p or n
		# points_trailer_dan2.append([curve_points[i][0], curve_points[i][1], (360-curve_points[i][2])%360, 'r'])
		deg = (curve_points[i][2])%360
		rad = np.deg2rad(deg)
		print(deg, rad)
		points_trailer_dan2.append([curve_points[i][0], curve_points[i][1], float(deg), 'r'])
		points_trailer_dan_trailer_h2.append([p_trailer_h[0], p_trailer_h[1], (360-(curve_points[i][2]+theta_rad))%360, 'r']) # p or n
	...
	A_150 = [points_trailer_dan[i] for i in indices]
	...
	for i in range(ext_step):
		...
		ph = end_node[2]
		points_trailer_dan_with_ext.append([px,py,ph])


	# print(points_trailer_dan_with_ext)

	################

	# Justin's html 3D view coordinate
	points_trailer_J = []
	# print("x--", points_trailer_dan2[0], points_trailer_dan2[-1])
	# p0 = [-points_trailer_dan2[0][1]+tx, points_trailer_dan2[0][0]+ty, (360-points_trailer_dan2[0][2])%360]
	# p1 = [-points_trailer_dan2[-1][1]+tx, points_trailer_dan2[-1][0]+ty, (360-points_trailer_dan2[-1][2])%360]
	# p2 = [-p2_loc[1]+tx, p2_loc[0]+ty, (360-p2_h)%360]
	# print(p0, p1, p2)
	# p00 = global_to_local_rotate(-veh_h, [p0[0], p0[1]])
	# p01 = global_to_local_rotate(-veh_h, [p1[0], p1[1]])
	# p02 = global_to_local_rotate(-veh_h, [p2[0], p2[1]])
	# print(p00, p01, p0[2]+veh_h, p1[2]+veh_h, veh_h) # vrkit loc

	

	# p2_loc = intersection_tra(
	# 	p0_3d=np.array(p00),
	# 	heading_start_deg=p0[2]+veh_h, 
	# 	p3_3d=np.array(p01), 
	# 	heading_end_deg=p1[2]+veh_h
	# )
	# p2_h = get_heading_p_to_p0(p01, p00)
	# # inter_point = [p2_loc[0], p2_loc[1], p2_h]
	...
	for i in range(len(points_trailer_dan_with_ext)):
		...
		# p = [-points_trailer_dan_with_ext[i][1]+tx, points_trailer_dan_with_ext[i][0]+ty, (360-points_trailer_dan_with_ext[i][2]-new_trailer_h)%360, 'r']
		p = [-points_trailer_dan_with_ext[i][1]+tx, points_trailer_dan_with_ext[i][0]+ty, (360-points_trailer_dan_with_ext[i][2]-new_trailer_h)%360, 'r']
		# p = [-points_trailer_dan_with_ext[i][1]+tx, points_trailer_dan_with_ext[i][0]+ty, 10, 'r']
		# print(p)

		p2 = global_to_local_rotate(-veh_h, [p[0], p[1]])
		p[0] = p2[0]
		p[1] = p2[1]
		points_trailer_J.append(p)
		# print(p)
	...
	points_trailer_J_x5.append(points_trailer_J[-1][:3])
	...
	# print(pointx5, '----')
	# path0, headings0 = generate_path([p1, p0], heading_scale=4.0, resolution=20)
	
	# res = []
	# for p,h in zip(path0, headings0):
	# 	res.append([p[0], p[1], float(h)])
	# print(res)
	
	# points_one_stage_den = [[xy[0], xy[1], h[0], "f"] for xy,h in zip(path0, headings0)]
		
	# with open('hummer_path/%s.txt'%fn1, 'w', encoding='utf-8') as f:
		# json.dump(points_one_stage_den, f, indent=2)
	

	## vrkit to can sensed coordinate
	can_heading_cali()
